import { FaBed, FaPen, FaToilet, FaTrash } from "react-icons/fa";
import { Link } from "react-router-dom";
const ListingItem = ({ id, data, onDelete, onEdit }) => {
  return (
    <div className="relative">
      ...
      >
        <img
          ...
          // src={data.imgUrls[data.imgUrls.length - 1]}
          src={data.imgUrls[0]}
          alt={data.name}
        />
      ...
      <div className="flex gap-3 absolute top-6 right-3">
        {onDelete && <FaTrash onClick={() => onDelete(data.id, data.name)} />}
        {onEdit && <FaPen onClick={() => onEdit(id)} />}
      </div>
    </div>
  );
};
